...
<html>
	<head>
		...
		function editar(id, valor){
			...
			`<form action="tarefa_controller.php?acao=atualizar&pagina=home" method="post" class='d-flex col-12'>
			...
			function remover(id){			
                $('#exclusao').modal('show')
                $('#sim').click(() =>{
                    window.location.href = 'home.php?acao=remover&id=' + id + '&pagina=home'
                })
			...
			function marcarRealizada(id){
				window.location.href = 'home.php?acao=marcar&id=' + id + '&pagina=home'
			}
			
			function apagarBoasVindas(){
				<?php if(isset($_SESSION['boasvindas'])){ ?>
					setTimeout(function(){
						let ajax = new XMLHttpRequest();
						ajax.open('GET', 'apagar_boasvindas.php')
	...
	<body onload="apagarBoasVindas()">
		...
		<?php if(isset($_SESSION['boasvindas'])) { ?>
			<div id="boasvindas" class="bg-info text-light d-flex justify-content-center">
				<h6>Seja bem vindo(a), <?=$nome?>!</h6>
			</div>
		<?php } ?>
						...
						<div class="row">
							<div class="col">
								...
								<div class="row mb-3 d-flex align-items-center tarefa" id="<?=$tarefa['id']?>">
									<div class="col-sm-9 mb-2"><?=$tarefa['tarefa']?><br><div style="font-size: 0.8em; float: right"><?=$data?></div>
                                        </div>                                        
									<div class="col-sm-3 mt-2 d-flex justify-content-between">
										<i class="fas fa-trash-alt fa-lg text-danger pointer" onclick="remover(<?=$tarefa['id']?>)"></i>											
